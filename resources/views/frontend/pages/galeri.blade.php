...
<script>
(function () {
    const items = Array.from(document.querySelectorAll('#gallery-lightbox .gallery-item img'));
    if (!items.length) return;
    items.forEach((img) => {
        img.addEventListener('click', () => {
            const overlay = document.createElement('div');
            overlay.style.cssText = 'position:fixed;inset:0;z-index:1000;background:rgba(15,23,42,.88);display:flex;align-items:center;justify-content:center;padding:1.5rem;cursor:zoom-out';
            const big = document.createElement('img');
            big.src = img.src;
            big.alt = img.alt || '';
            big.style.cssText = 'max-width:min(1100px,96vw);max-height:90vh;border-radius:16px;box-shadow:0 25px 60px rgba(0,0,0,.45);object-fit:contain';
