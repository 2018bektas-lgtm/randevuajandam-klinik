...
<div class="mb-6 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
    <div class="min-w-0">
        <div class="flex flex-wrap items-center gap-2 text-xs mb-2">
            <a href="{{ route('panel.egitimler.index') }}" class="font-bold text-slate-500 hover:text-[#C96A2B] transition">← Eğitimler</a>
            @if(! $tumu && $egitim)
                <span class="text-slate-300">/</span>
                <a href="{{ route('panel.egitimler.edit', $egitim->id) }}" class="font-semibold text-slate-600 hover:text-[#C96A2B] truncate max-w-[200px]">{{ $egitim->baslik }}</a>
                <span class="text-slate-300">/</span>
    ...
    <div class="flex flex-wrap gap-2">
        @if(! $tumu && $egitim)
            <a href="{{ route('panel.egitimler.basvurular.tumu') }}"
            ...
            <a href="{{ route('panel.egitimler.edit', $egitim->id) }}"
        ...
        @else
            <a href="{{ route('panel.egitimler.create') }}"
...
@if(session('basari') || session('basarili'))
    <div class="mb-4 p-3.5 rounded-2xl bg-emerald-50 border border-emerald-100 text-sm text-emerald-800">{{ session('basari') ?: session('basarili') }}</div>
@endif
@if(session('hata'))
        ...
        <div class="flex flex-wrap gap-2">
            @foreach(['' => 'Tümü', 'beklemede' => 'Beklemede', 'onaylandi' => 'Onaylı', 'reddedildi' => 'Red', 'iptal' => 'İptal'] as $k => $v)
                @php
                    ...
                    $href = $tumu
                        ? route('panel.egitimler.basvurular.tumu', $params)
                        : route('panel.egitimler.basvurular', array_merge(['id' => $egitim->id], $params));
                @endphp
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div class="flex items-end gap-2 sm:col-span-2 lg:col-span-1">
                ...
                @if(request()->hasAny(['q','durum','ucret','egitim_id']))
                    <a href="{{ $tumu ? route('panel.egitimler.basvurular.tumu') : route('panel.egitimler.basvurular', $egitim->id) }}"
        ...
        <article class="eb-card">
            <div class="p-4 sm:p-5 grid lg:grid-cols-12 gap-4">
                ...
                <div class="lg:col-span-4 min-w-0">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            @if($tumu && $eg)
                                <a href="{{ route('panel.egitimler.basvurular', $eg->id) }}"
                ...
                <div class="lg:col-span-4 space-y-3">
                    <form method="POST" action="{{ route('panel.egitimler.basvuru.durum', [$b->egitim_id, $b->id]) }}" class="space-y-2 p-3 rounded-xl bg-slate-50 border border-slate-100">
                        @csrf
                    ...
                    @if($b->ucret_durumu !== 'yok' && $b->ucret_durumu !== 'odendi')
                        <form method="POST" action="{{ route('panel.egitimler.basvuru.odeme', [$b->egitim_id, $b->id]) }}"
