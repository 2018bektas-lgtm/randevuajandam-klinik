...
<style>
    .select2-container--default .select2-selection--single {
        height: 42px; border: 1px solid #e7e5e4; border-radius: .85rem; padding: .35rem .5rem;
        display: flex; align-items: center;
    }
    .select2-container--default .select2-selection--single .select2-selection__rendered {
        line-height: 1.4; font-size: .875rem; color: #1c1917; padding-left: .25rem;
    }
    .select2-container--default .select2-selection--single .select2-selection__arrow { height: 40px; }
    .select2-dropdown { border-color: #e7e5e4; border-radius: .85rem; overflow: hidden; }
    .select2-container--default .select2-results__option--highlighted[aria-selected] { background: #C96A2B; }
</style>
@endpush
...
@php
    ...
    if (! empty($egitim?->kapak)) {
        $kapakUrl = str_starts_with((string) $egitim->kapak, 'http')
            ? $egitim->kapak
            : media_url($egitim->kapak);
    }
    $dt = function ($v) {
        if (! $v) return '';
        try {
            return \Illuminate\Support\Carbon::parse($v)->format('Y-m-d\TH:i');
        } catch (\Throwable) {
            return '';
        }
    };
...
@if(session('basari') || session('basarili'))
    <div class="mb-5 p-3.5 rounded-2xl bg-emerald-50 border border-emerald-100 text-sm text-emerald-800 flex items-start gap-2.5">
        ...
        <span>{{ session('basari') ?: session('basarili') }}</span>
    </div>
@endif
@if(session('hata'))
    <div class="mb-5 p-3.5 rounded-2xl bg-red-50 border border-red-100 text-sm text-red-700">{{ session('hata') }}</div>
@endif
@if($errors->any())
...
<div class="mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
    <div class="min-w-0">
        <div class="flex flex-wrap items-center gap-2 mb-2">
            <a href="{{ route('panel.egitimler.index') }}" class="text-xs font-bold text-slate-500 hover:text-[#C96A2B] transition">← Eğitimler</a>
            @if($isEdit)
    ...
    <div class="flex flex-wrap items-center gap-2">
        @if($isEdit)
            ...
            <a href="{{ route('panel.egitimler.basvurular', $egitim->id) }}"
...
<form method="POST"
      action="{{ $isEdit ? route('panel.egitimler.update', $egitim->id) : route('panel.egitimler.store') }}"
                    ...
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="datetime-local" name="baslangic_at"
                                   value="{{ old('baslangic_at', $dt($egitim->baslangic_at ?? null)) }}"
                                   class="egf-input">
                        </div>
                        <div>
                            <label class="egf-label">Bitiş</label>
                            <input type="datetime-local" name="bitis_at"
                                   value="{{ old('bitis_at', $dt($egitim->bitis_at ?? null)) }}"
                                   class="egf-input">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="basvuru_bitis_at"
                                   value="{{ old('basvuru_bitis_at', $dt($egitim->basvuru_bitis_at ?? null)) }}"
                                   class="egf-input">
                        </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 z-30 border-t border-slate-200 bg-white/95 backdrop-blur-md">
        <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 py-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2 ml-auto">
                <a href="{{ route('panel.egitimler.index') }}"
...
@if($isEdit)
    <form id="silForm" method="POST" action="{{ route('panel.egitimler.destroy', $egitim->id) }}"
