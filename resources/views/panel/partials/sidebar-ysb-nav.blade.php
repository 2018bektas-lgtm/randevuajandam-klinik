...
@php
    ...
    $ysbRouteIs = function ($match) {
        if (is_array($match)) {
            return request()->routeIs(...$match);
        }
        $parts = array_values(array_filter(array_map('trim', explode('|', (string) $match))));

        return $parts === [] ? false : request()->routeIs(...$parts);
    };
@endphp

<nav class="ysb-nav" aria-label="Panel menu">
    @if($ysbDash)
        @php $dashActive = $ysbRouteIs($ysbDash['match'] ?? ''); @endphp
    ...
    @foreach($ysbGroups as $gi => $group)
        ...
        @endif
        @php
            $groupActive = collect($group['items'])->contains(fn ($i) => $ysbRouteIs($i['match'] ?? ''));
        @endphp
                ...
                <div class="ysb-panel-inner">
                    <ul class="ysb-list">
                        @foreach($group['items'] as $item)
                            @php $active = $ysbRouteIs($item['match'] ?? ''); @endphp
                            <li>
                                <a href="{{ $item['href'] }}"
