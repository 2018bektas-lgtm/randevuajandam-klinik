...
@section('icerik')
                            ...
                            <tr class="hover:bg-[#FAFAFA]/75 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    @if(($talep->gorusme_tipi ?? 'yuz_yuze') === 'online')
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-sky-50 text-sky-700 border border-sky-200 font-display">📹 Online</span>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-slate-50 text-slate-600 border border-slate-200 font-display">Yüz yüze</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <!-- Approve Action Form -->
                                        <form action="{{ route('panel.randevular.durum', $talep->id) }}" method="POST" class="inline">
                                            @csrf
    ...
    <script>
        function talepReddetModalAc(talep) {
            ...
            let route = "{{ route('panel.randevular.durum', ':id') }}";
