...
@section('icerik')
    <div class="mb-4">
        <form method="GET" action="{{ route('panel.hastalar') }}" class="flex flex-wrap gap-2">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Ad, telefon veya e-posta ara..."
                   class="flex-1 min-w-[200px] px-3.5 py-2.5 rounded-xl border border-[#E5E7EB] text-xs focus:outline-none focus:border-[#C96A2B]">
            <button type="submit" class="px-4 py-2.5 rounded-xl bg-[#C96A2B] text-white text-xs font-bold font-display">Ara</button>
            @if(request('q'))
                <a href="{{ route('panel.hastalar') }}" class="px-4 py-2.5 rounded-xl border text-xs font-bold text-slate-500">Temizle</a>
            @endif
        </form>
    </div>
