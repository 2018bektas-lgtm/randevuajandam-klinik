...
@section('baslik', 'Web Sitesi Entegrasyonu')
@section('sayfa_baslik', 'Web Sitesi Entegrasyonu')
...
<div class="space-y-8">
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="border-t border-gray-100 pt-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        @if($apiKey)
                            <form action="{{ route('panel.web-sitesi.api-anahtari') }}" method="POST" onsubmit="return confirm('API anahtarlarını yeniden oluşturmak mevcut sitenizin veri senkronizasyonunu kesecektir. Emin misiniz?')">
                                @csrf
                    ...
                    @if($apiKey)
                        <div class="space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="p-4 border border-gray-200 rounded-xl bg-gray-50 flex items-center justify-between">
                                    <div class="flex-1 min-w-0 pr-4">
                                        <span class="text-xs text-gray-400 block font-medium uppercase tracking-wider">Secret Key (HMAC İmza Anahtarı)</span>
                                        <div class="flex items-center gap-2 mt-1">
                                            <code class="text-sm font-semibold text-gray-800 font-mono block truncate" id="secretKeyVal">****************************************</code>
                                            <input type="hidden" id="realSecretKey" value="{{ $apiKey->secret_key }}">
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-1">
                                        <button onclick="toggleSecretKey()" class="p-2 text-gray-500 hover:text-gray-800 hover:bg-gray-200 rounded-lg transition-all" title="Göster/Gizle" id="toggleSecretBtn">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    @else
                        <div class="p-6 border border-dashed border-gray-200 rounded-xl text-center">
                            ...
                            <form action="{{ route('panel.web-sitesi.api-anahtari') }}" method="POST">
                                @csrf
    ...
    @else
        ...
        <form action="{{ route('panel.web-sitesi.kurulum') }}" method="POST" class="space-y-8">
            @csrf
