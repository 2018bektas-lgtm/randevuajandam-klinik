...
@section('baslik', 'Blog YazÄ±sÄ± DÃ¼zenle - Hekim Paneli')
@section('sayfa_baslik', 'Blog YazÄ±sÄ± DÃ¼zenle')

@section('icerik')
<div class="mb-6">
    <a href="{{ route('panel.bloglar') }}" class="inline-flex items-center gap-1 text-xs text-[#6B7280] hover:text-[#C96A2B] font-semibold transition-colors font-display">
...
<div class="bg-white rounded-2xl border border-[#E5E7EB] p-8 shadow-sm">
    <form action="{{ route('panel.bloglar.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <div class="space-y-1.5">
                    <label for="baslik" class="block text-[10px] font-bold text-[#1F2937] uppercase tracking-wider font-display">Makale BaÅŸlÄ±ÄŸÄ±</label>
                    <input type="text" name="baslik" id="baslik" value="{{ old('baslik', $blog->baslik) }}" required placeholder="BaÅŸlÄ±k girin..."
                ...
                <!-- Content -->
                <div class="space-y-1.5">
                    <label for="icerik" class="block text-[10px] font-bold text-[#1F2937] uppercase tracking-wider font-display">Makale Ä°Ã§eriÄŸi</label>
                    <textarea name="icerik" id="icerik" rows="12" required placeholder="Makalenizin iÃ§eriÄŸini buraya yazÄ±n..."
                              class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-[#E5E7EB] text-[#111827] focus:outline-none focus:border-[#C96A2B] focus:ring-1 focus:ring-[#C96A2B] text-xs transition-all">{{ old('icerik', $blog->icerik) }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-1.5 flex items-center justify-between p-4.5 rounded-xl bg-slate-50 border border-slate-100">
                    <div class="max-w-[150px]">
                        <label class="block text-[10px] font-bold text-[#1F2937] uppercase tracking-wider font-display">YayÄ±n Durumu</label>
                        <span class="text-[9px] text-[#6B7280]">Profil sayfanÄ±zda hemen yayÄ±nlansÄ±n mÄ±?</span>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer select-none">
                        <input type="checkbox" name="aktif_mi" id="aktif_mi" value="1" class="sr-only peer" {{ old('aktif_mi', $blog->aktif_mi) ? 'checked' : '' }}>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <div class="w-full h-44 bg-slate-50 rounded-2xl border border-dashed border-[#E5E7EB] flex items-center justify-center overflow-hidden relative group">
                            <img id="previewBlogImage" src="{{ $blog->resim ? media_url($blog->resim) : '' }}" alt="Ã–nizleme" class="w-full h-full object-cover {{ $blog->resim ? '' : 'hidden' }}">
                            <div id="uploadPlaceholder" class="text-center p-6 flex flex-col items-center gap-1.5 cursor-pointer {{ $blog->resim ? 'hidden' : '' }}" onclick="document.getElementById('resim').click()">
                            ...
                            <button type="button" class="absolute right-3 top-3 bg-black/60 hover:bg-black/80 text-white p-1.5 rounded-lg text-xs {{ $blog->resim ? '' : 'hidden' }}" id="removeImageBtn" onclick="clearSelectedImage()">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                ...
                <div class="bg-slate-50/50 rounded-2xl border border-[#E5E7EB] p-5 space-y-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="meta_baslik" id="meta_baslik" value="{{ old('meta_baslik', $blog->meta_baslik) }}" placeholder="Arama motoru baÅŸlÄ±ÄŸÄ±..."
                               class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-[#E5E7EB] text-[#111827] focus:outline-none focus:border-[#C96A2B] focus:ring-1 focus:ring-[#C96A2B] text-xs transition-all">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="meta_aciklama" id="meta_aciklama" rows="3" placeholder="Makalenin kÄ±sa arama sonucu Ã¶zeti..."
                                  class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-[#E5E7EB] text-[#111827] focus:outline-none focus:border-[#C96A2B] focus:ring-1 focus:ring-[#C96A2B] text-xs transition-all resize-none">{{ old('meta_aciklama', $blog->meta_aciklama) }}</textarea>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <!-- Hidden Input -->
                        <input type="hidden" name="meta_anahtar_kelimeler" id="meta_anahtar_kelimeler" value="{{ old('meta_anahtar_kelimeler', $blog->meta_anahtar_kelimeler) }}">
