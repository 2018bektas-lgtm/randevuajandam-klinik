...
<div class="mb-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
    ...
    <a href="{{ route('panel.bloglar.create') }}" class="px-5 py-2.5 bg-[#C96A2B] hover:bg-[#B55A20] text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-200 shadow-sm font-display flex items-center justify-center gap-2">
...
<div class="bg-white rounded-2xl border border-[#E5E7EB] shadow-[0_4px_24px_rgba(31,41,55,0.04)] overflow-hidden">
    @if($bloglar->isEmpty())
        <div class="p-12 text-center">
            ...
            <a href="{{ route('panel.bloglar.create') }}" class="inline-flex mt-5 px-4 py-2 bg-[#1F2937] hover:bg-[#111827] text-white text-xs font-bold rounded-lg transition-colors font-display">
                    ...
                    @foreach($bloglar as $blog)
                        <tr class="hover:bg-slate-50/40 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($blog->resim)
                                    <img src="{{ media_url($blog->resim) }}" alt="{{ $blog->baslik }}" class="w-16 h-10 object-cover rounded-lg border border-slate-100 shadow-sm">
                                @else
                            ...
                            <td class="px-6 py-4">
                                <div class="max-w-md">
                                    <a href="{{ '#' }}" target="_blank" class="block font-bold text-[#111827] text-sm hover:text-[#C96A2B] transition-colors truncate font-display">
                                        {{ $blog->baslik }}
                                    </a>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-[#4B5563] font-medium font-display">
                                {{ $blog->okunma_sayisi ?? 0 }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-xs text-[#6B7280]">
                                {{ \Illuminate\Support\Carbon::parse($blog->created_at ?? now())->translatedFormat('d M Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($blog->aktif_mi)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium space-x-2">
                                <a href="{{ route('panel.bloglar.edit', $blog->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#E5E7EB] rounded-lg text-xs font-bold text-[#4B5563] hover:text-[#C96A2B] hover:bg-[#FFF7ED] hover:border-[#E7B58A]/30 transition-all font-display">
                                ...
                                <form action="{{ route('panel.bloglar.destroy', $blog->id) }}" method="POST" class="inline" onsubmit="return onayModalAc(event, this, 'Bu blog yazısını silmek istediğinize emin misiniz? Bu işlem geri alınamaz.');">
                                    @csrf
                                    @method('DELETE')
