...
@section('icerik')
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm">
        <div class="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
            <a href="{{ route('panel.finans') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">
                📊 Genel Bakış
            </a>
            <a href="{{ route('panel.finans.gelirler') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.gelirler') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">
                💵 Gelir Kayıtları
            </a>
            <a href="{{ route('panel.finans.giderler') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.giderler') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">
                💸 Gider Kayıtları
            </a>
            <a href="{{ route('panel.finans.hasta-bakiyeleri') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.hasta-bakiyeleri') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">
                👥 Hasta Bakiyeleri
            </a>
            <a href="{{ route('panel.finans.kategoriler') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.kategoriler') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">
                🏷️ Kategoriler
            </a>
        </div>
        <div>
            <a href="{{ route('panel.finans.rapor-pdf') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold bg-white border border-[#E5E7EB] hover:border-[#C96A2B] text-[#4B5563] hover:text-[#C96A2B] transition-all shadow-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('panel.finans.gelirler') }}" class="text-xs font-semibold text-[#C96A2B] hover:underline">Tümünü Gör</a>
            </div>
                    ...
                    <div class="py-3.5 flex items-center justify-between">
                        <div>
                            <span class="block text-sm font-semibold text-[#111827]">
                                @if(!empty($odeme->hasta))
                                    {{ trim(($odeme->hasta->ad ?? '') . ' ' . ($odeme->hasta->soyad ?? '')) ?: ($odeme->hasta->ad_soyad ?? 'Hasta') }}
                                @else
                                    Serbest Gelir
                                @endif
                            </span>
                            <span class="block text-xs text-[#6B7280] mt-0.5">
                                {{ $odeme->hizmet->ad ?? 'Hizmet Dışı Serbest Kayıt' }}
                                • {{ \Illuminate\Support\Carbon::parse($odeme->odeme_tarihi ?? $odeme->created_at)->format('d.m.Y') }}
                            </span>
                        </div>
        ...
        <div class="p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('panel.finans.giderler') }}" class="text-xs font-semibold text-[#C96A2B] hover:underline">Tümünü Gör</a>
            </div>
            <div class="divide-y divide-[#F3F4F6]">
                    ...
                    <div class="py-3.5 flex items-center justify-between">
                        <div>
                            ...
                            <span class="block text-xs text-[#6B7280] mt-0.5">
                                ...
                                @endphp
                                {{ $kategoriIsimleri[$gider->kategori] ?? 'Diğer' }} • {{ \Illuminate\Support\Carbon::parse($gider->tarih)->format('d.m.Y') }}
                            </span>
                        </div>
                        <div class="text-right">
                            ...
                            @if($gider->belge_yolu)
                                <a href="{{ media_url($gider->belge_yolu) }}" target="_blank" class="inline-flex items-center gap-1 text-[10px] font-semibold text-[#C96A2B] hover:underline mt-0.5">
                                    📄 Belgeyi Gör
                                </a>
                            @endif
                        </div>
