...
@section('icerik')
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm">
        <div class="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
            <a href="{{ route('panel.finans') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">📊 Genel Bakış</a>
            <a href="{{ route('panel.finans.gelirler') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.gelirler') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">💵 Gelir Kayıtları</a>
            <a href="{{ route('panel.finans.giderler') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.giderler') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">💸 Gider Kayıtları</a>
            <a href="{{ route('panel.finans.hasta-bakiyeleri') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.hasta-bakiyeleri') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">👥 Hasta Bakiyeleri</a>
            <a href="{{ route('panel.finans.kategoriler') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.kategoriler') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">🏷️ Kategoriler</a>
        </div>
    ...
    @if(session('basari'))
        <div class="mb-6 p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm font-semibold">{{ session('basari') }}</div>
    @endif
    ...
    <div class="p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm mb-6">
        <form method="GET" action="{{ route('panel.finans.giderler') }}" class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
            <div>
                ...
                <tbody class="divide-y divide-[#E5E7EB] text-sm text-[#111827]">
                    @forelse($giderler as $gider)
                        <tr class="hover:bg-[#FAFAFA]/50 transition-colors">
                            ...
                            <td class="p-4">
                                @if($gider->belge_yolu)
                                    <a href="{{ media_url($gider->belge_yolu) }}" target="_blank" class="inline-flex items-center gap-1.5 text-xs font-semibold text-[#C96A2B] hover:underline">
                            ...
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <form action="{{ route('panel.finans.giderler.destroy', $gider->id) }}" method="POST" onsubmit="return confirm('Bu gider kaydını silmek istediğinize emin misiniz?')" class="inline">
                                        @csrf @method('DELETE')
