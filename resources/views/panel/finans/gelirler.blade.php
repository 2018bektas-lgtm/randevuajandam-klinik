...
@section('icerik')
    ...
    <div class="p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm mb-6">
        <form method="GET" action="{{ route('panel.finans.gelirler') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 items-end">
            <div>
                                                ...
                                                @foreach($odeme->kalemler as $kalem)
                                                    ...
                                                    <tr class="hover:bg-white/60">
                                                        ...
                                                        <td class="px-4 py-2.5 text-right">
                                                            <form action="{{ route('panel.finans.gelirler.kalem.destroy', [$odeme->id, $kalem->id]) }}" method="POST" onsubmit="return confirm('Bu ödeme kalemini silmek istediğinize emin misiniz?')" class="inline">
                                                                @csrf @method('DELETE')
