...
@section('icerik')
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm">
        <div class="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
            ...
            <a href="{{ route('panel.finans.hasta-bakiyeleri') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-150 {{ request()->routeIs('panel.finans.hasta-bakiyeleri') || request()->routeIs('panel.finans.hasta-hesap') ? 'bg-[#C96A2B] text-white shadow-sm' : 'bg-[#FAFAFA] text-[#4B5563] hover:bg-[#F3F4F6]' }}">
                👥 Hasta Bakiyeleri
            </a>
        </div>
    </div>
    ...
    <div class="p-6 rounded-2xl bg-white border border-[#E5E7EB] shadow-sm mb-6">
        <form method="GET" action="{{ route('panel.finans.hasta-bakiyeleri') }}" class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <tr class="hover:bg-[#FAFAFA]/50 transition-colors">
                            ...
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-3 flex-wrap">
                                    <a href="{{ route('panel.finans.hasta-hesap', $hasta->id) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#C96A2B] text-white text-xs font-bold hover:bg-[#b05c24]" title="Hasta hesabı">
                                        Hesap sayfası
                                    </a>
                                    <a href="{{ route('panel.finans.gelirler', ['hasta_id' => $hasta->id]) }}" class="inline-flex items-center gap-1 text-xs font-bold text-[#C96A2B] hover:underline" title="Gelir listesi">
                                        Gelirler
                                    </a>
                                </div>
                            </td>
