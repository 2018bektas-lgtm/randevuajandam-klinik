...
<div class="bg-white rounded-2xl border border-[#E5E7EB] p-8 shadow-sm">
    <form action="{{ route('panel.hizmetler.update', $hizmet->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <div class="w-full h-44 bg-slate-50 rounded-2xl border border-dashed border-[#E5E7EB] flex items-center justify-center overflow-hidden relative group">
                            <img id="previewBlogImage" src="{{ $hizmet->resim ? media_url($hizmet->resim) : '' }}" alt="Ã–nizleme" class="w-full h-full object-cover {{ $hizmet->resim ? '' : 'hidden' }}">
