...
<head>
    ...
    <title>İki Adımlı Doğrulama · Klinik Hekim Paneli</title>
...
<div class="w-full max-w-md bg-white border border-slate-200 rounded-3xl shadow-sm p-8">
    <p class="text-[10px] font-bold uppercase tracking-wider text-[#C96A2B] font-display">Güvenlik</p>
    <h1 class="mt-2 text-xl font-bold text-slate-900 font-display">İki adımlı doğrulama</h1>
    ...
    <form method="POST" action="{{ route('panel.two-factor.challenge.post') }}" class="mt-6 space-y-4">
        @csrf
        <div>
            ...
            <input type="text" name="code" id="code" inputmode="numeric" autocomplete="one-time-code" autofocus required
                   placeholder="000000"
                   class="w-full px-4 py-3 rounded-xl border border-slate-200 text-center text-lg tracking-[0.3em] font-semibold focus:outline-none focus:border-[#C96A2B] focus:ring-1 focus:ring-[#C96A2B]">
        </div>
        <button type="submit"
    ...
    <form method="POST" action="{{ route('panel.two-factor.challenge.cancel') }}" class="mt-3">
        @csrf
