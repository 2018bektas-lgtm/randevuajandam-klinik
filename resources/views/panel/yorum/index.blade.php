...
<div class="space-y-8">
    ...
    <!-- Filters -->
    <div class="bg-white border border-[#E5E7EB] rounded-2xl p-5 shadow-sm">
        <form method="GET" action="{{ route('panel.yorumlar') }}" class="flex flex-wrap items-end gap-4">
            <div class="space-y-1">
            ...
            @if(request()->hasAny(['durum', 'puan']))
                <a href="{{ route('panel.yorumlar') }}" class="px-4 py-2 border border-[#E5E7EB] hover:bg-slate-50 text-[#6B7280] font-bold text-xs uppercase tracking-wider rounded-xl transition-all font-display">
                    Temizle
                </a>
            @endif
        </form>
    ...
    <div class="space-y-4">
        @forelse($yorumlar as $yorum)
            <div class="bg-white border border-[#E5E7EB] rounded-2xl p-6 shadow-sm space-y-4 hover:shadow-md transition-shadow">
                ...
                @else
                    <!-- Reply Form -->
                    <form action="{{ route('panel.yorumlar.yanitla', $yorum->id) }}" method="POST" class="flex flex-col sm:flex-row gap-3">
                        @csrf
