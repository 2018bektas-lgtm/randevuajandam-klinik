...
class SiteAyarlariController extends Controller
{
    ...
    /**
     * Özel sayfa yönetimi (KVKK, gizlilik, kampanya vb.).
     * Menü dropdown'ından seçilir; footera checkbox ile eklenir.
     */
    ...
    public function sayfaKaydet(Request $request, ?int $id = null)
    {
        ...
        $slug = $slugInput !== ''
            ? Str::slug($slugInput)
            : SitePage::makeSlug($data['baslik'], $id);
        if ($slug === '') {
        ...
        try {
            ...
            // sanitizer yoksa ham HTML (CKEditor çıktısı)
        }
        ...
        return redirect()
            ...
            ->with('basari', $msg.' Menüye eklemek için Site Ayarları → Menü → “Sayfa: …”. Footer için “Footer’da göster” işaretli olmalı.');
    }

    public function sayfaSil(int $id)
    {
        $page = SitePage::query()->findOrFail($id);
        // Menüde bu sayfaya bağlı satırları temizle
        SiteMenuItem::query()
